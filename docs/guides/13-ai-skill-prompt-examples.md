...
### 작업 마감·깃반영·복습 — `asak-signoff`, `asak-git-publish`, `asak-study`

```text
퇴근 정리 - [날짜 / 기능]
현재 Git·코드·Figma 근거를 확인해 워크로그와 필요한 문서만 갱신해줘.
검증 완료·미확인·다음 작업을 구분하고, 소스코드와 Git 원격 상태는 바꾸지 마.
```

```text
깃반영 - [저장소 / 포함 파일]
브랜치, 한글 커밋 제목, push, main 병합, local·remote 브랜치 삭제 중 승인한 단계만 실행해줘.
먼저 Git 상태·원격·변경 파일·테스트 결과를 보여주고, force·자동 충돌 해결은 하지 마.
```
