...
> **2026-07-16 override:** do not clone or modify `ASAK-front` for current implementation work. The current local target structure is `C:\ASAK-workspace\ASAK`, `ASAK-Kiosk`, `ASAK-Admin`, and `ASAK-back`. Kiosk remote migration is blocked pending owner confirmation; see [current status baseline](../wiki/current-status-baseline.md).
