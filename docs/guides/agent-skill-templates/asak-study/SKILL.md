...
3. 구현 완료, mock 기반, 실제 API 연결, DB 반영, 미검증 상태를 분리한다. 소스코드와 Git 이력은 수정하지 않는다. 학습 결과는 아래 결과 기록 파일로 남긴다.
...
작업이 끝나면 대상 저장소에 `docs/ai-reports/YYYY-MM-DD/<스킬이름>-<짧은주제>.md`를 남긴다. 폴더가 없으면 만든다.

- 보고서에는 작업 범위, 확인·수정한 파일, 결론, 검증 근거, 남은 위험, 다음 작업을 적는다.
- 코드·DB·원격 Git을 바꾸지 않은 분석 작업도 결과 보고서는 남긴다.
- 비밀값, 토큰, 개인정보, 원본 오류 로그 전체는 기록하지 않는다.
- 결과 파일을 만들었다고 자동 commit·push하지 않는다. Git 반영은 사용자의 별도 승인 범위만 따른다.
- 같은 요청을 이어서 처리하면 같은 보고서를 갱신하고, 새 작업이면 새 파일을 만든다.
