...
>
> **2026-08-19 실측 검증:** 운영 DB의 `SHOW CREATE VIEW` 와 대조했다. 이 문서에 적힌 22개가
> 실제 뷰 전부이며 누락·잉여는 없다. 20개는 정의가 토큰 단위까지 같았고,
...
>
> **`vw_order_live` 상태 필터 확대 (2026-08-19):** 주방 보드가 결제 대기(`READY`) 주문도 보여주도록
> 상태 필터를 `IN ('RECEIVED','PREPARING','READY')` 로 넓혔다. 운영 DB 적용 확인됨. 현재 보드에
> READY 3건이 함께 잡힌다 (PREPARING 9 · RECEIVED 1 · READY 3 = 13행).
