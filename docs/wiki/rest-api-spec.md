...
> **2026-07-14 표준:** 모든 서비스 endpoint는 `/api`로 시작하며 JSON property는 `camelCase`다. 제공된 DevCopilot MCP 연결 URL은 서비스 API endpoint가 아니다. DB의 `snake_case` 컬럼명은 API에 노출하지 않는다. 상세 전환 기준은 [2026-07-14 피드백 반영 기준](../_archive/team-audits-2026-07/2026-07-14-feedback-resolution.md)을 따른다.
