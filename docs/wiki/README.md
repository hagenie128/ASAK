# ASAK DevCopilot Wiki Index

> workspace 2 · https://devcopilot.ai.kr/workspace/2/wiki

## 산출물 Wiki (학원 체크리스트)

| Wiki 제목 | 로컬 md | DevCopilot URL |
|-----------|---------|----------------|
| ASAK 요구사항 정의서 | [requirements-definition.md](./requirements-definition.md) | https://devcopilot.ai.kr/workspace/2/wiki/9 |
| ASAK 사용자 시나리오 명세 | [user-scenarios.md](./user-scenarios.md) | https://devcopilot.ai.kr/workspace/2/wiki/10 |
| ASAK 화면 설계 및 Figma 연동 | [screen-design-figma.md](./screen-design-figma.md) | https://devcopilot.ai.kr/workspace/2/wiki/5 |
| ASAK DB 설계 테이블 정의서 | [db-table-definition.md](./db-table-definition.md) | https://devcopilot.ai.kr/workspace/2/wiki/11 |
| ASAK REST API 명세서 | [rest-api-spec.md](./rest-api-spec.md) | https://devcopilot.ai.kr/workspace/2/wiki/12 |
| ASAK WBS 및 일정 계획 | [wbs-schedule.md](./wbs-schedule.md) | https://devcopilot.ai.kr/workspace/2/wiki/13 |
| ASAK QA 테스트 케이스 | [qa-test-cases.md](./qa-test-cases.md) | https://devcopilot.ai.kr/workspace/2/wiki/14 |
| ASAK 회의록 및 최종 배포 검증 | [meeting-deliverables-checklist.md](./meeting-deliverables-checklist.md) | https://devcopilot.ai.kr/workspace/2/wiki/15 |

## 기존 Wiki

| Wiki 제목 | md |
|-----------|-----|
| ASAK 종합 기획서 요약 | [asak-planning-summary.md](./asak-planning-summary.md) | — |
| ASAK 기술스택 요약 | [tech-stack-summary.md](./tech-stack-summary.md) | — |
| ASAK DB/API 개요 | [db-api-overview.md](./db-api-overview.md) | — |

## 업로드

```powershell
python asak-data/scripts/upload_wiki.py --title "ASAK 요구사항 정의서" --file docs/wiki/requirements-definition.md
```
