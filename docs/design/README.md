...
- `FIGMA_TOKEN_SYNC_2026-07-20.md`: Figma 토큰 동기화 인수인계 기록
...
완료 감사는 [`_archive/audits/`](_archive/audits/), 완료 프롬프트는 [`_archive/figma-prompts/`](_archive/figma-prompts/), 과거 병합·후보 비교는 [`_archive/legacy-ds02-scr/`](_archive/legacy-ds02-scr/)에 보관합니다. 플러그인 소스와 자산은 실행 자산이므로 이 cleanup에서 이동하거나 수정하지 않습니다.

```powershell
git status -- docs/design docs/_archive
```
