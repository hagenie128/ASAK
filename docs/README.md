...
> **2026-07-23:** 현재 구현 상태는 `planning/current-implementation-map-2026-07-16.md` · [구현 현황 요약](wiki/current-status-baseline.md), 실행 작업 분해표 정본은 [`wiki/wbs.md`](wiki/wbs.md)입니다.
...
- **보관 문서:** `_archive/` — 삭제하거나 정본으로 되돌리지 않음
...
12. [보관 문서](_archive/) — 한물간 문서로 실행에 사용하지 않음
...
| `product_bible` | 제품 기준 팩 1~12 (`_archive` 제외) | `#canonical` |
| `operations` | 설치·회의록 정본 | `#current` |
| `design` | Figma 실행 스택 (`_archive` 별도) | `#current` |
...
| `_archive` | 종료 이력·내보내기·구 가이드 | `#archive` |
