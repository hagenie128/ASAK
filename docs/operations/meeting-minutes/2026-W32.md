...
| 팀   | 김나연, 이하진                                |
| 목차 | [← 이전](2026-W31.md) · [목록](README.md) · [다음 →](2026-W33.md) |
...
1. `ASAK-Kiosk/src/adapters/orderAdapter.js:20` 당시 경로가 `return payload` — W31에서 정한 필드명 변환이 **경계에 자리만 있고 미구현**. 실연동 시 `orderNumber`/`totalPrice`/`waitingCount`가 그대로 흘러감
