...
### Admin 실행 순서 메모 (2026-07-21)

- 먼저 주문 운영(`WBS2-035~037`)을 끝내고,
- 다음으로 운영 설정(`WBS2-038~040`)을 처리하고,
- 그 다음 조회성 화면(`WBS2-034`, `WBS2-041~043`)을 묶어서 정리한다.
- `WBS2-044~045` 공통 상태/QA는 마지막에 붙인다.
- 아직 손대지 않은 화면은 placeholder / 정적 UI 상태를 유지하고, 구현 시작 시점에만 page 분리/추출을 진행한다.
