...
| 항목 | 내용 |
| --- | --- |
| 공부 주제 | 관리자 메뉴 관리 화면의 목록·상세·옵션 그룹 표시 흐름 |
| 대상 화면 | `SCR-016` Admin Menu Management |
| 확인 기준일 | `2026-08-06` |
| 확인한 범위 | Frontend / Backend / API 명세 / DB 조회 SQL / Screen Bible |
| 이 문서의 결론 | 관리자 메뉴 관리 화면은 백엔드 페이지네이션 목록을 기준으로 카드를 렌더링하고, 선택된 `menuId`로 상세 API를 다시 조회해 우측 패널을 채우는 구조이며, 옵션 그룹은 `vw_menu_opt_policy_json`의 가공 결과를 화면용 문구로 바꿔 표시하는 방향으로 정리 중이다. |

### 한 줄 결론

오늘 확인한 관리자 메뉴 관리 흐름의 핵심은 `목록(PageResult)과 상세(menuId 단건 조회)를 분리`하고, `옵션 그룹은 내부 policy 이름이 아니라 화면용 그룹 이름으로 표시`하도록 정리하는 것이다.
...
부분 확인. `MenuListPanel`은 빈 목록 시 `"조건에 맞는 메뉴가 없습니다"`를 표시한다. 다만 `menus === null` 분기를 쓰고 있어 빈 배열 중심 구조와 완전히 일치하지는 않는다.
...
직접적인 비활성 제어는 메뉴 목록 카드에 없다. 옵션 그룹 배지와 조회 중심 UI라 버튼 중복 클릭 방지는 오늘 범위에서 핵심이 아니었다.
...
1. `ASAK-Admin/src/components/admin/MenuEditPanel.jsx`
2. `ASAK-back/src/main/java/com/asak/admin/dto/response/MenuDetailResponse.java`
