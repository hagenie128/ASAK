...
1. **Batch 2 시작 record: API-016 `장바구니 검증`** — server Cart API의 `NEEDS_CONFIRMATION`, canonical endpoint와 DTO/View Model 분리를 개별 재검증한다.
2. API-002~005/009 및 DTO·DB 관련 P0 record를 10개 이하 batch로 실제 열람한다. `GET /api/kiosk/menuList`, `GET /api/kiosk/menuDetail/{menuId}`, `POST /api/kiosk/orders`, `POST /api/kiosk/payments`, `PATCH /api/admin/soldOut`의 canonical 후보와 legacy path를 구분한다.
3. P0 API/DTO/DB가 끝난 뒤 Batch 3 WBS·Requirements로 진행한다. P1/P2 회의록·워크로그·Archive는 시작하지 않는다.
...
| `39251ef0-4f0b-8127-a34b-e3534f376a63` | API-016 Cart 검증 | P0 | OPENED | 변경 없음 | Notion Web 개별 열람 | server Cart validation endpoint가 남아 있어 `CART_POLICY_MISMATCH`; Batch 2에서 `NEEDS_CONFIRMATION`으로 계약 정리 |
