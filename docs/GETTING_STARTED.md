...
|------------------|--------|---------|
| `c:\ha-team` | [ASAK](https://github.com/hagenie128/ASAK) | 문서, 데이터 파이프라인, 팀 가이드 |
...
`ha-team` 안의 `frontend/`, `backend/` 폴더는 **구조 참고용**에 가깝습니다.  
실제 키오스크 개발은 `ASAK-front`와 `ASAK-back`에서 합니다.

> 작업 후 `ha-team`에 다시 합칠 필요는 **없습니다.** 어디서 작업했으면 그 저장소에만 push 하면 됩니다.
...
`ha-team`의 **간단한 데이터 뷰어**(`frontend/run_viewer.py`)는 JSON 파일만으로도 동작합니다.  
...
**성공 확인:** `c:\ha-team\data-pipeline\phase1\output\` 아래에 `menus.json` 등 파일이 생깁니다.
...
> `ha-team\backend\` 는 골격만 있는 참고 폴더입니다. **실제 실행은 `c:\ASAK-back`에서** 하세요.
...
## 5단계 (선택): ha-team 간단 데이터 뷰어

크롤링 JSON만 빠르게 볼 때:

```powershell
cd c:\ha-team\frontend
```
...
2. **저장소 3개를 혼동하지 마세요.** 프론트 수정은 `ASAK-front`, 백엔드는 `ASAK-back`, 문서/파이프라인은 `ha-team`.
3. **포트 충돌:** 8080(백엔드), 5173(프론트 Vite), 8765(뷰어)가 이미 쓰이면 "Address already in use" 에러 → 다른 프로그램 종료 또는 포트 변경.
4. **`sync_phase1_data_to_front.bat`** 는 기본 경로가 `c:\greens`로 되어 있을 수 있습니다. `c:\ha-team`을 쓰면 경로를 수정하세요.
...
- [ ] `c:\ha-team`, `c:\ASAK-front`, `c:\ASAK-back` 클론 완료
