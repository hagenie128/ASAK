...
def build_meta() -> dict:
    meta: dict = {"generated": True, "files": {}}
    for name in (
        "menus.json",
        "dressings.json",
        "store_menus.json",
    ...
    if (OUTPUT_DIR / "menus.json").exists():
        menus = json.loads((OUTPUT_DIR / "menus.json").read_text(encoding="utf-8"))
        meta["stats"] = {
            "menus": len(menus),
            "with_dressing": sum(
                1
    ...
    if (OUTPUT_DIR / "dressings.json").exists():
        dressings = json.loads((OUTPUT_DIR / "dressings.json").read_text(encoding="utf-8"))
        meta["stats"]["dressings"] = dressings.get("count", len(dressings.get("items", [])))
    if (OUTPUT_DIR / "store_menus.json").exists():
        stores = json.loads((OUTPUT_DIR / "store_menus.json").read_text(encoding="utf-8"))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="샐러디 데이터 뷰어")
    ...
    if not OUTPUT_DIR.exists():
        raise SystemExit(f"output 폴더가 없습니다. 먼저 크롤러를 실행하세요.")
