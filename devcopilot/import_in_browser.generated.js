...
const SCENARIOS = [
  {
    ...
    "normalFlow": "홈(SCR-001)·매장/포장 선택 → 카테고리·메뉴 선택(SCR-003) → 옵션 선택(SCR-004) → 장바구니·주문확인 컨펌 팝업(SCR-005) → 결제(SCR-007) → 주문 완료(SCR-008)",
  ...
  },
  {
    ...
    "postCondition": "결제 완료 및 주문번호 표시, 화면 전환 4회 이내 목표 (2026-07-06: SCR-002·006 병합, 주문확인은 SCR-005 컨펌 팝업)",
    "normalFlow": "메뉴 선택 → 추천조합 기본값 → 장바구니(SCR-005) 컨펌 팝업 확인 → 결제(SCR-007) → 완료(SCR-008)",
    "alternativeFlow": "없음",
  ...
  },
  {
    ...
    "normalFlow": "장바구니·주문확인 컨펌 팝업(SCR-005) → API-005 주문 생성 → 결제(SCR-007) → 승인 → 주문 완료(SCR-008)",
  ...
  },
  {
    "scenarioId": "SC-014",
    "title": "매장/포장 여부 선택",
    "preCondition": "키오스크 홈(SCR-001) 화면 진입",
    "postCondition": "주문에 orderType(EAT_IN/TAKE_OUT) 구분값이 저장되어 장바구니(SCR-005)까지 유지",
    "normalFlow": "홈(SCR-001)에서 매장/포장 선택 → orderType을 메뉴·옵션·장바구니(SCR-005)까지 유지 → 주문 생성(API-005) 시 orderType 반영",
  ...
  },
  {
    ...
    "preCondition": "Week 5 MVP(8/1) 또는 최종 발표(9/2) 전 전체 데모 환경이 준비되어 있음",
    "postCondition": "고객 주문 완료와 관리자 주문 확인/상태 변경까지 한 번에 시연 가능",
    "normalFlow": "1. 홈(SCR-001)에서 매장/포장 선택 → 2. 메뉴·옵션(SCR-003/004) → 3. 장바구니·주문확인 컨펌 팝업(SCR-005) → 4. 결제(SCR-007) → 5. 완료(SCR-008) → 6. 관리자 확인(SCR-009/010)",
