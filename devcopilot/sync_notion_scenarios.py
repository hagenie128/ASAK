...
UPDATES: dict[str, dict] = {
    "SC-001": {
        ...
        "normal_flow": "홈 → 먹고가기/포장 → 카테고리·메뉴 선택 → 옵션 선택 → 장바구니(SCR-005) → 주문 확인(SCR-006) → 결제(SCR-007) → 주문 완료(SCR-008)",
    ...
    "SC-002": {
        "normal_flow": "메뉴 선택 → 추천조합 기본값 → 장바구니(SCR-005) → 주문 확인(SCR-006) → 결제(SCR-007) → 완료(SCR-008)",
        "post_condition": "결제 완료 및 주문번호 표시, 화면 전환 5회 이내 목표 (2026-07-05 수정: 4단계→5회, 주문확인 화면 반영)",
        "mermaid_script": (
            "flowchart TD\n"
    ...
    "SC-004": {
        "normal_flow": "장바구니(SCR-005) → 주문 확인(SCR-006) → API-005 주문 생성 → 결제(SCR-007) → 승인 → 주문 완료(SCR-008)",
        "mermaid_script": (
            "flowchart TD\n"
    ...
    "SC-024": {
        "normal_flow": (
            "1. 홈(SCR-001) 주문 시작 → 2. 먹고가기/포장(SCR-002) → 3. 메뉴·옵션(SCR-003/004) "
            "→ 4. 장바구니·주문확인(SCR-005/006) → 5. 결제(SCR-007) → 6. 완료(SCR-008) "
            "→ 7. 관리자 주문 확인·상태변경(SCR-009/010)"
        ),
        "mermaid_script": (
            "flowchart TD\n"
            "    A[홈 SCR-001] --> B[먹고가기/포장 SCR-002]\n"
