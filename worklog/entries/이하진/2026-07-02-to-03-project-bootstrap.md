...
- 9주 전체 범위를 한 번에 구현하려 하지 않고, 1차 MVP를 명확히 정의해 팀 작업 순서를 고정한다.
...
- 1차 MVP를 "고객 주문 흐름이 끝까지 동작하는 상태"로 정의하고, React 화면 → Spring Boot/MySQL → API 연동 → 테스트/발표 순으로 통합 단계를 잡았다. Day 10 발표에서는 고객 주문 흐름과 관리자 주문 확인을 우선하고, 부가 기능은 기본 동작 이후로 분리했다.
...
- 사용한 AI: 사후 워크로그 정리 시 Git 커밋과 변경 파일을 대조하는 보조. MVP·역할 계획 초안 구조화.
- 사람이 수행한 부분: 폴더 역할, 팀 workflow, 템플릿 내용과 적용 범위 결정. 계획 초안에서 AI에게 "할 일 목록"만 만들게 하지 않고 MVP·후순위·블로커·검증 조건을 나누게 요청했고, 발표 범위·팀원 역할·최종 우선순위는 AI 결과를 그대로 쓰지 않고 실제 팀 상황에 맞게 직접 좁혔다.
...
- 이 시점부터 워크로그를 단순 출석 기록이 아니라, 이후 "왜 이렇게 설계했는지"를 복원할 수 있는 근거로 사용하기 시작했다. `커밋 존재 ≠ 완료`, `UI 존재 ≠ 기능 완료` 같은 증거 중심 기록 방식은 이 판단에서 출발했다.
