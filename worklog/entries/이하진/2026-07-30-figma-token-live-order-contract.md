...
- 반영 이력: `e9786c5`(최신 Figma 기준과 환불 계약 정리), `565cf59`(관리자 Live 주문 UI와 상태 처리 추가), `5932a55`(키오스크 Figma 토큰과 간격 정리)
...
- 완료 판정: 문서·코드·스타일 커밋과 Admin/Kiosk 빌드는 확인했다. API/DB 통합, 브라우저 동작, Figma 화면별 픽셀 대조는 미완료다.
...
4. 카드의 상태 변경·취소 클릭은 확인 절차 후 API 요청을 시도하고, 성공 toast 뒤 `refresh({ showLoading: false })`로 목록을 다시 읽도록 구성돼 있다.
...
### 이슈 1 — Live 주문 행동 URL의 계약 불일치

- 확인한 코드: `changeOrderStatus(orderId, action)`은 `/admin/orders/${orderId}/${action}`을 호출하지만, 현재 Screen Bible은 `PATCH /api/admin/orders/{orderId}/status`를 기준으로 둔다.
- 확인한 코드: `cancelOrder(orderId)`는 문자열 리터럴이 아닌 `cancel` 식별자를 URL 템플릿에 사용한다.
- 영향: lint/build는 통과해도 클릭 시 서버 경로·요청 값이 어긋날 수 있다.
- 필요한 해결: 백엔드 Controller의 실제 mapping과 요청 body를 확인한 뒤 API 모듈을 계약에 맞게 팀원이 수정하고 Bruno·브라우저로 정상/실패를 검증한다.

### 이슈 2 — API·DB 구현 및 환불 검증 미완료

- 문서에는 Live 조회와 완료 주문 환불의 정책을 기록했지만, 현재 기준 문서는 Admin 상태 변경·취소·환불 Controller mapping이 미구현/미검증으로 표시한다.
- 필요한 해결: Controller → Service → Mapper → DB 순서로 구현 여부를 확인하고, `APPROVED → REFUNDED`, `COMPLETED` 유지, 중복 환불 `409`을 실제로 검증한다.

### 이슈 3 — Kiosk Variable 설계 결정 필요
...
- Admin lint: 오류 0건, 기존/현재 미사용 변수 경고 3건. `LiveOrderPreview`의 `readLiveFixture`도 경고 대상이다.
- Admin build: Vite 프로덕션 빌드 성공. 500kB 초과 번들 경고가 남았다.
- Kiosk build: Vite 프로덕션 빌드 성공. 500kB 초과 번들 경고가 남았다.
- ASAK-back의 스테이징된 미커밋 변경은 변경 파일이 존재하지만, 이 날짜의 커밋·검증 근거에는 포함하지 않았다.
...
|---|---|---|---|
| Git 이력 | `e9786c5`, `565cf59`, `5932a55`의 작성자·변경 파일·diff 대조 | 7월 30일 이하진 커밋 확인 | 원격 반영·PR/CI |
| Figma | 0718 파일의 Admin `134:10606`, Kiosk `134:7720` 메타데이터 조회 | 캔버스 구조·프레임 존재 확인 | 화면별 최신 Node 확정, 픽셀 비교, Variable 변경 |
| Admin lint | `npm.cmd --prefix C:\ASAK-workspace\ASAK-Admin run lint` | 오류 0건, 경고 3건 | 클릭·네트워크·API 응답 |
| Admin build | `npm.cmd --prefix C:\ASAK-workspace\ASAK-Admin run build` | 성공, 대형 번들 경고 | 브라우저 UI·상태 변경·취소 |
| Kiosk build | `npm.cmd --prefix C:\ASAK-workspace\ASAK-Kiosk run build` | 성공, 대형 번들 경고 | 화면별 시각 QA·고대비·Variable 재바인딩 |

## 11. 다음 작업 / 검증 체크리스트

- [ ] Admin backend의 Live 조회·상태 변경·취소 mapping과 프론트 URL/body를 한 계약으로 확정한다.
...
- 관리자 Live 주문 보드의 조회 상태·카드 배치·확인 UX와 키오스크 토큰 적용을 커밋 및 빌드까지 확인했다. 다만 상태 변경·취소 API와 Figma 화면별 시각 검증은 별도 단계로 남겼다.
