# 2026-07-31 키오스크 공통 UI·레이아웃·고객 API·주문·메뉴

> **일일 기록:** [2026-07-31 daily](../../daily/김나연/2026-07-31.md)
...
- 브랜치: fix/admin-menu-pagination~7, fix/admin-menu-pagination~8, remotes/origin/ny/api-connection~10
- 관련 이슈/PR/화면: API-005
- 작업 유형: `feature` / `fix`
- 구현 근거: `1f64a3c`, `f1554e0`, `cf45aa2`
...
- ASAK-Kiosk / ASAK-back에서 확인된 당일 작업을 **키오스크 공통 UI·레이아웃 · 고객 API·주문·메뉴** 단위로 정리한다.
...
- **키오스크 공통 UI·레이아웃:**
  - `1f64a3c` fix : env 파일명 수정
  - 주요 파일: `.env.example` (`1f64a3c`)
- **고객 API·주문·메뉴:**
  - `f1554e0` update: feature/user/API-005 주문 생성 로직 구현 추가
  - `cf45aa2` fix: feature/user/API-005 UserOrserMapper.xml insert문에 id 추가 안한 오류 해결
  - 주요 파일: `src/main/java/com/asak/common/exception/ErrorCode.java`, `src/main/java/com/asak/user/dto/order/internal/OrderInsertDto.java`, `src/main/java/com/asak/user/dto/order/internal/OrderItemInsertDto.java`, `src/main/java/com/asak/user/dto/order/response/CreateOrderResponse.java`, `src/main/java/com/asak/user/mapper/UserOrderMapper.java`, `src/main/java/com/asak/user/service/UserOrderService.java`, `src/main/resources/mappers/UserOrderMapper.xml` (`f1554e0`, `cf45aa2`)
...
- **키오스크 공통 UI·레이아웃:** Header/Footer/Stepper/공통 CSS로 화면 간 레이아웃을 맞췄다. 대표 파일: `.env.example`.
- **고객 API·주문·메뉴:** Controller→Service→Mapper(XML)→DTO 흐름으로 고객 메뉴/주문 API를 보강했다. 대표 파일: `ErrorCode.java`, `OrderInsertDto.java`, `OrderItemInsertDto.java`, `CreateOrderResponse.java`.
...
|---|---|---|
| 당일 커밋 | 3건 (`1f64a3c` … `cf45aa2`) | 각 저장소 `git log --since/--until` |
| 키오스크 공통 UI·레이아웃 | 커밋 1건 · 파일 1+ | `.env.example` |
| 고객 API·주문·메뉴 | 커밋 2건 · 파일 7+ | `src/main/java/com/asak/common/exception/ErrorCode.java` |
...
- 확인 결과:
  - 2026-07-31 작성자 커밋 **3건**과 영역별 파일 목록을 확인했다.
  - 이 entry 작성/보강 시점에는 자동 테스트·실서버 호출·E2E 성공 결과를 새로 확보하지 않았다.

## 11. 포트폴리오용 요약

2026-07-31 김나연 작업은 ASAK-Kiosk / ASAK-back에서 키오스크 공통 UI·레이아웃 · 고객 API·주문·메뉴 범위를 커밋 3건으로 진행한 기록이다. 변경 파일과 영역 경계를 기준으로 설명하고, 통합 테스트·실DB·브라우저 검증은 완료로 과장하지 않았다.

## 12. 첨부하면 좋은 자료

- 일일 기록: [2026-07-31 daily](../../daily/김나연/2026-07-31.md)
- 관련 커밋: `1f64a3c`, `f1554e0`, `cf45aa2`
- 변경이 큰 경로: `.env.example`
