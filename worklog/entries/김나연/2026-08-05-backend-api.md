# 2026-08-05 키오스크 메뉴·옵션·상세·고객 API·주문·메뉴

> **일일 기록:** [2026-08-05 daily](../../daily/김나연/2026-08-05.md)
...
- 브랜치: remotes/origin/docs/doc-tone-cleanup~6^2~1, remotes/origin/ny/api-connection~5
- 관련 이슈/PR/화면: API-003
- 작업 유형: `fix` / `docs`
- 구현 근거: `dfff72d`, `4cef988`
...
- ASAK-Kiosk / ASAK-back에서 확인된 당일 작업을 **키오스크 메뉴·옵션·상세 · 고객 API·주문·메뉴** 단위로 정리한다.
...
- **키오스크 메뉴·옵션·상세:**
  - `dfff72d` fix: data.optionGroups 호출 오타 부분 수정
  - 주요 파일: `src/pages/kiosk/MenuDetailPage.jsx` (`dfff72d`)
- **고객 API·주문·메뉴:**
  - `4cef988` update: api-003 기존 명세서 수정(제외 재료 반영시킴) 및 수정된 부분 반영
  - 주요 파일: `src/main/java/com/asak/user/dto/menu/IngredientResponse.java`, `src/main/java/com/asak/user/dto/menu/Nutrition.java`, `src/main/java/com/asak/user/mapper/UserMenuMapper.java`, `src/main/resources/mappers/UserMenuMapper.xml` (`4cef988`)
...
- **고객 API·주문·메뉴:** Controller→Service→Mapper(XML)→DTO 흐름으로 고객 메뉴/주문 API를 보강했다. 대표 파일: `IngredientResponse.java`, `Nutrition.java`, `UserMenuMapper.java`, `UserMenuMapper.xml`.
...
|---|---|---|
| 당일 커밋 | 2건 (`dfff72d` … `4cef988`) | 각 저장소 `git log --since/--until` |
| 키오스크 메뉴·옵션·상세 | 커밋 1건 · 파일 1+ | `src/pages/kiosk/MenuDetailPage.jsx` |
| 고객 API·주문·메뉴 | 커밋 1건 · 파일 4+ | `src/main/java/com/asak/user/dto/menu/IngredientResponse.java` |
...
- 확인 결과:
  - 2026-08-05 작성자 커밋 **2건**과 영역별 파일 목록을 확인했다.
  - 이 entry 작성/보강 시점에는 자동 테스트·실서버 호출·E2E 성공 결과를 새로 확보하지 않았다.

## 11. 포트폴리오용 요약

2026-08-05 김나연 작업은 ASAK-Kiosk / ASAK-back에서 키오스크 메뉴·옵션·상세 · 고객 API·주문·메뉴 범위를 커밋 2건으로 진행한 기록이다. 변경 파일과 영역 경계를 기준으로 설명하고, 통합 테스트·실DB·브라우저 검증은 완료로 과장하지 않았다.

## 12. 첨부하면 좋은 자료

- 일일 기록: [2026-08-05 daily](../../daily/김나연/2026-08-05.md)
- 관련 커밋: `dfff72d`, `4cef988`
