...
> 기준일: 2026-07-13 · 이 문서는 **정책형 옵션 구조와 짧은 명명 규칙이 적용된 MySQL DB**를 기준으로 한다. JSON 파일명과 로컬 적재 스크립트의 전환은 별도 호환 작업으로 관리한다.

## 정합성 점검 및 명명 규칙

### 확인 결과

현재 JSON 시드는 이전 옵션 구조인 `menu_option_group`, `menu_option`을 보관한다. 운영 설계는 이를 재사용 정책 구조인 `option_policy`, `option_policy_item`, `menu_option_policy`, `menu_option_override`로 대체한다. 따라서 정책 테이블은 별도 JSON 시드가 아니라 기존 옵션 JSON에서 생성한다.

추가로 다음 차이를 확인했다.

- `seed/manifest.json`의 `common_code` 건수는 32건이나, DB 설계의 현재 운영 기준은 35건이다.
- `menu._source_ref`는 원본 추적용 JSON 메타데이터다. MySQL 적재에서는 제외해야 하며, 업무 테이블 컬럼으로 사용하지 않는다.
- SQLite 샘플의 주문 컬럼(`order_number`, `order_status_id`, `unit_price`, `approved_at`)은 이 문서와 MySQL 기준(`order_no`, `status_id`, `price`, `paid_at`)에 맞춰 정리 대상이다.
- 이 문서에서 말하는 운영 24개는 기본 운영 목록 20개와 옵션 정책 4개를 합한 값이다. legacy 백업 2개까지 포함한 실제 물리 테이블 수는 26개다.

### 짧은 명명 규칙

- 테이블은 소문자 snake_case를 사용하고, `option`은 `opt`, `ingredient`는 `ing`, `nutrition`은 `nutr`, `payment`는 `pay`로 줄인다.
- FK는 참조 테이블의 짧은 이름 뒤에 `_id`를 붙인다. 예: `opt_item_id`, `ing_id`.
- 상태 boolean은 `is_` 접두어를 없애 `active`, `sold_out`, `required`, `default`, `recommended`로 쓴다. 단, SQL 예약어 충돌 가능성이 있는 `default`는 `is_default`를 유지한다.
- 원본 추적 정보는 업무 테이블이 아닌 import 메타데이터로 분리한다.

### 테이블명 전환표 (MySQL 적용 완료)

| 변경 전 명칭 | 현재 DB 명칭 | 비고 |
| --- | --- | --- |
| `menu_nutrition` | `menu_nutr` | 메뉴 영양정보 |
| `ingredient` | `ing` | 재료 마스터 |
| `ingredient_allergen` | `ing_allergen` | 재료-알레르기 연결 |
| `menu_ingredient` | `menu_ing` | 메뉴 기본 재료 |
| `option_group` | `opt_group` | 옵션 그룹 |
| `option_item` | `opt_item` | 옵션 항목 |
| `option_item_component` | `opt_item_comp` | 옵션 구성품 |
| `option_policy` | `opt_policy` | 재사용 옵션 정책 |
| `option_policy_item` | `opt_policy_item` | 정책별 옵션 항목 |
| `menu_option_policy` | `menu_opt_policy` | 메뉴-옵션 정책 연결 |
| `menu_option_override` | `menu_opt_override` | 메뉴별 옵션 예외 |
| `payment_method_config` | `pay_method_cfg` | 결제수단 표시 설정 |
| `menu_option_group_legacy_20260710` | `menu_opt_grp_legacy_20260710` | 백업 전용 |
| `menu_option_legacy_20260710` | `menu_opt_legacy_20260710` | 백업 전용 |

### 주요 컬럼명 전환표 (MySQL 적용 완료)
...
> MySQL 이름 전환은 완료되었으며 행 데이터는 삭제하지 않았다. 실행 전 스키마 백업은 `asak-data/schema-backups/`에 저장한다. JSON 파일명, MySQL DDL/적재기, SQLite 샘플 DDL, API 쿼리는 별도 호환 작업으로 전환한다.
...
- DB 엔진: MySQL 계열
- 기준 seed: `asak-data/seed-v3/manifest.json` (짧은 명칭·정책형 JSON)
- 이전 seed: `asak-data/seed/manifest.json` (변환 원본/호환 보관)
- MySQL 명칭 마이그레이션: `asak-data/scripts/migrate_short_names_mysql.py`
- 짧은 명칭 seed 적재 스크립트: `asak-data/scripts/load_short_name_seed_mysql.py`
- 기존 스키마/seed 적재 스크립트: `asak-data/scripts/load_seed_mysql.py` (명칭 전환 전 버전)
- 기존 옵션 정책 테이블 적재 스크립트: `asak-data/scripts/apply_option_policy_mysql.py` (명칭 전환 전 버전)
- 관리자 매출 View 생성 스크립트: `asak-data/scripts/create_sales_views_mysql.py`
- 기본 운영 테이블: 20개
- 옵션 정책 테이블: 4개 (운영 합계 24개)
- legacy 백업 테이블: 2개
- 관리자 통계 View: 4개
